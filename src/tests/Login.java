...
public class Login extends BaseTest{
	...
	private void setEmailForRegistration() throws InterruptedException {
		...
		driver.findElement(By.id("email")).sendKeys(getRandom(names)+random.nextInt(900)+"@" + getRandom(domain)+".com");
    ...
    {
    	ExtentTestManager.getTest().setDescription("Test Description: All fields are filled correctly, no hard errors are shown");
    	...
    	waitforNoErrors.withMessage("No unique email id could be generated , Alternative: Clear DB with testing registered emails and then run these tests");
    	...
    	waitForToastMessage.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".notification-content")));
    	waitForToastMessage.withMessage("Toast Message is still shown");
    	waitForToastMessage.until(ExpectedConditions.invisibilityOfElementLocated(By.cssSelector(".notification-content")));
